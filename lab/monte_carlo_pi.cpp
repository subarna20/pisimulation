...
// Number of samples
constexpr int size_n = 40000;  // Must be greater than size_wg
// Size of parallel work groups
constexpr int size_wg = 48;
...
constexpr int radius = img_dimensions / 3;

// Returns the pixel index corresponding to a set of simulation coordinates
SYCL_EXTERNAL int GetPixelIndex(double x, double y, double z) {
  int img_x = x * radius + radius;
  int img_y = y * radius + radius;
  int img_z = z * radius + radius;
  return img_z * img_dimensions + img_y + img_x;
}
...
void DrawPlot(rgb image_plot[]) {
  for (int i = 0; i < img_dimensions * img_dimensions; ++i) {
    ...
    double z = (double)(i / img_dimensions - radius) / radius;
    // draw the circumference of the circle
    if ((x * x + y * y + z * z) > 1 - circle_outline && (x * x + y * y + z * z) < 1) {
      ...
      h.parallel_for(nd_range<1>(num_wg * size_wg, size_wg),
                     ...
                     [=](nd_item<1> it, auto& total_acc) {
                       ...
                       if (i < size_n) {  // Only runs if a work item's ID has a
                         ...
                         double z = coords_acc[i].z;

                         // Check if coordinates are bounded by a circle of
                         // radius 1
                         double hypotenuse_sqr = x * x + y * y + z * z;
                         if (hypotenuse_sqr <= 1.0) {  // If bounded
                           // increment total
                           total_acc += 1;
                           // Draw sample point in image plot
                           imgplot_acc[GetPixelIndex(x, y, z)].red = 0;
                           imgplot_acc[GetPixelIndex(x, y, z)].green = 255;
                           imgplot_acc[GetPixelIndex(x, y, z)].blue = 0;
                         } else {
                           // Draw sample point in image plot
                           imgplot_acc[GetPixelIndex(x, y, z)].red = 255;
                           imgplot_acc[GetPixelIndex(x, y, z)].green = 0;
                           imgplot_acc[GetPixelIndex(x, y, z)].blue = 0;
                         }
                       }
                     });
